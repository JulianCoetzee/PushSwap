...
/*   Updated: 2019/08/20 14:52:00 by jcoetzee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	(find_common(t_moves *moves))
{
	...
	{
		c = (move->a_moves > moves->b_moves ? moves->b_moves : moves->a_moves);
		if (c)
		{
...
static t_moves	*calc_moves_a_to_b(t_stacks *stacks, int pos)
{
	...
	moves->b_moves = find_place(stacks->stack_b, stacks->size_b,
			stacks->stack_a[pos], &(moves->b_moves));
